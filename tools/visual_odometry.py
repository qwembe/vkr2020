...
class VisualOdometry:
    ...
    def getAbsoluteScale(self, frame_id):  # specialized for KITTI odometry dataset
        ...
        mul = 10
        x_prev = float(ss[1]) * mul
        y_prev = float(ss[2]) * mul
        z_prev = float(ss[3]) * mul
        ss = self.annotations[frame_id].strip().split()
        x = float(ss[1]) * mul
        y = float(ss[2]) * mul
        z = float(ss[3]) * mul
        self.trueX, self.trueY, self.trueZ = x, y, z
        print(frame_id, self.trueX,self.trueY)
        return np.sqrt((x - x_prev) * (x - x_prev) + (y - y_prev) * (y - y_prev) + (z - z_prev) * (z - z_prev)) * 100
